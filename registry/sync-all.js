...
if (!TOKEN) {
  ...
  console.error('Usage: FIGMA_TOKEN=figd_xxx node registry/sync-all.js [ds2026|fm|all]');
  process.exit(1);
}
...
async function getComponentSetDetails(fileKey, nodeIds) {
  ...
  for (let i = 0; i < nodeIds.length; i += 50) {
    ...
    if (i + 50 < nodeIds.length) {
      console.log(`    Fetched ${Math.min(i + 50, nodeIds.length)}/${nodeIds.length} node details...`);
    }
  }
  return results;
}
...
function generateComponentReference(fileInfo, componentSets, components, libId, registryComponents) {
  const md = [];
  const isDS = libId === 'ds2026';
  const title = isDS ? 'Actian Design System 2026' : 'Fat Marker Kit';
  const filename = isDS ? 'ds2026-component-reference.md' : 'fm-component-catalog.md';
  ...
  md.push(`${componentSets.length} component sets, ${components.length} individual components.`);
  md.push('');
    ...
    for (const cs of pageComps.sort((a, b) => a.name.localeCompare(b.name))) {
      ...
      if (reg && Object.keys(reg.variants).length > 0) {
        ...
        for (const [axis, values] of Object.entries(reg.variants)) {
          varParts.push(`**${axis}:** ${values.map(v => '`' + v + '`').join(' · ')}`);
        }
      ...
      if (reg && reg.textProperties.length > 0) {
        const textNames = reg.textProperties.map(t => '`' + t.split('#')[0] + '`');
...
async function syncLibrary(libId) {
  ...
  console.log(`   ${fileInfo.pages.length} pages, last modified ${fileInfo.lastModified}`);

  // Components
  console.log('\n2. Fetching components...');
  const { componentSets, components } = await getComponents(lib.fileKey);
  console.log(`   ${componentSets.length} component sets, ${components.length} components`);
  ...
  for (const cs of componentSets) {
    ...
    const { variants, variantShortNames, textProperties, booleanProperties } = detail
      ? extractFromNode(detail)
      : { variants: {}, variantShortNames: {}, textProperties: [], booleanProperties: [] };
  ...
  console.log(`   ${Object.keys(registryComponents).length} total components in registry`);

  // Generate reference doc
  console.log('\n6. Generating reference doc...');
  const { content, filename } = generateComponentReference(fileInfo, componentSets, components, libId, registryComponents);
...
async function main() {
  ...
  const targets = arg === 'all' ? ['ds2026', 'fat-marker']
    : arg === 'ds2026' ? ['ds2026']
    : arg === 'fm' ? ['fat-marker']
    : [arg];
  ...
  console.log(`Registry: ${Object.keys(allComponents).length} components → ${registryPath}`);
  ...
  console.log(`  registry/component-registry.json  (${Object.keys(allComponents).length} components)`);
  if (tokenCount > 0) console.log(`  registry/token-map.json           (${tokenCount} tokens)`);
  for (const libId of targets) {
    const filename = libId === 'ds2026' ? 'ds2026-component-reference.md' : 'fm-component-catalog.md';
